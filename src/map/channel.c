...
int channel_join(struct Channel *channel, struct map_session_data *sd) {
	...
	if(channel_haspcbanned(channel,sd)==1){ //banned ?
		sprintf(output, msg_txt(1438),channel->name); //You cannot join the '%s' channel because you've been banned from it
...
int channel_mjoin(struct map_session_data *sd) {
	...
	if( !( map[sd->bl.m].channel->opt & CHAN_OPT_ANNOUNCE_JOIN ) ) {
		char mout[60];
		sprintf(mout, msg_txt(1435),Channel_Config.map_chname,map[sd->bl.m].name); // You're now in the '#%s' channel for '%s'.
...
int channel_display_list(struct map_session_data *sd, char *options){
	...
	if( options[0] != '\0' && strcmpi(options,"colors") == 0 ) {
		...
		clif_displaymessage(sd->fd, msg_txt(1475)); // ---- Joined Channels ----
		if(!sd->channel_count)
			clif_displaymessage(sd->fd, msg_txt(1476)); //You have not joined any channel yet
		else {
			for(k=0; k<sd->channel_count; k++){
				channel = sd->channels[k];
				sprintf(output, msg_txt(1409), channel->name, db_size(channel->users));// - #%s (%d users)
				clif_displaymessage(sd->fd, output);
			}
		}
	}
		...
		clif_displaymessage(sd->fd, msg_txt(1410)); // ---- Public Channels ----
		if( Channel_Config.map_enable ) {
			sprintf(output, msg_txt(1409), Channel_Config.map_chname, map[sd->bl.m].channel ? db_size(map[sd->bl.m].channel->users) : 0);// - #%s ( %d users )
			clif_displaymessage(sd->fd, output);
		}
		if( Channel_Config.ally_enable && sd->status.guild_id ) {
			struct guild *g = sd->guild;
			if( !g ) return -1; //how can this happen if status.guild_id true ?
			sprintf(output, msg_txt(1409), Channel_Config.ally_chname, db_size(((struct Channel *)g->channel)->users));// - #%s ( %d users )
			clif_displaymessage(sd->fd, output);
		}
...
int channel_pccreate(struct map_session_data *sd, char *chname, char *chpass){
	...
	if(res==0){ //succes
		...
		if( !( channel->opt & CHAN_OPT_ANNOUNCE_JOIN ) ) {
			sprintf(output, msg_txt(1403),chname); // You're now in the '%s' channel.
			clif_displaymessage(sd->fd, output);
		}
		...
		case -3: sprintf(output, msg_txt(1436), CHAN_NAME_LENGTH); break;// Channel pass can't be higher then %d.
...
int channel_pcdelete(struct map_session_data *sd, char *chname){
	...
	sprintf(output, msg_txt(1477),chname); //Channel %s deleted
	clif_displaymessage(sd->fd, output);
...
int channel_pcleave(struct map_session_data *sd, char *chname){
	...
	sprintf(output, msg_txt(1426),chname); // You've left the '%s' channel.
	clif_displaymessage(sd->fd, output);
	return 0;
}
...
int channel_pcjoin(struct map_session_data *sd, char *chname, char *pass){
	...
	if(channel){
		if(channel_haspc(channel,sd)==1) {
			sprintf(output, msg_txt(1434),chname); // You're already in the '%s' channel.
			clif_displaymessage(sd->fd, output);
			return -1;
		}
			...
			if(strcmp(channel->pass,pass) != 0){ //wrong pass entry
				if( pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) {
					...
					sprintf(output, msg_txt(1401),chname,"@Join"); // '%s' Channel is password protected (usage: %s <#channel_name> <password>)
					clif_displaymessage(sd->fd, output);
					return -1;
				}
			}
		...
		sprintf(output, msg_txt(1400),chname,"@Join"); // Unknown Channel '%s' (usage: %s <#channel_name>)
		clif_displaymessage(sd->fd, output);
		return -1;
	}
...
int channel_pccolor(struct map_session_data *sd, char *chname, char *color){
	...
	if( k >= Channel_Config.colors_count ) {
		sprintf(output, msg_txt(1411), color);// Unknown color '%s'.
		clif_displaymessage(sd->fd, output);
		return -1;
	}
	channel->color = k;
	sprintf(output, msg_txt(1413),chname,Channel_Config.colors_name[k]);// '%s' channel color updated to '%s'.
	clif_displaymessage(sd->fd, output);
	return 0;
}
...
int channel_pcbind(struct map_session_data *sd, char *chname){
	...
	sd->gcbind = channel;
	sprintf(output, msg_txt(1431),chname); // Your global chat is now binded to the '%s' channel.
	clif_displaymessage(sd->fd, output);
	return 0;
}
...
int channel_pcunbind(struct map_session_data *sd){
	...
	if( sd->gcbind == NULL ) {
		clif_displaymessage(sd->fd, msg_txt(1432));// Your global chat is not binded to any channel.
		return -1;
	}
	sprintf(output, msg_txt(1433),sd->gcbind->name); // Your global chat is now unbinded from the '#%s' channel.
	clif_displaymessage(sd->fd, output);
	sd->gcbind = NULL;
	return 0;
}
...
int channel_pcban(struct map_session_data *sd, char *chname, struct map_session_data *tsd, int flag){
	...
	if(flag != 2 && flag != 3){
		...
		if(!tsd || pc_has_permission(tsd, PC_PERM_CHANNEL_ADMIN) ) {
			clif_displaymessage(sd->fd, msg_txt(1464)); // Ban failed, not possible to ban/unban this user.
			return -1;
		}

		banned = channel_haspcbanned(channel,tsd);
		if(!flag &&  banned==1) {
			sprintf(output, msg_txt(1465), tsd->status.name);// Player '%s' is already banned from this channel
			clif_displaymessage(sd->fd, output);
			return -1;
		}
		else if(flag==1 && banned==0) {
			sprintf(output, msg_txt(1440), tsd->status.name);//  1440: Player '%s' is not banned from this channel
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	}
		...
		if( !db_size(channel->banned) ) {
			sprintf(output, msg_txt(1439), chname);// Channel '%s' has no banned players
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	}
	...
	case 0:
		...
		sprintf(output, msg_txt(1437),tsd->status.name,chname); // Player '%s' has now been banned from '%s' channel
		break;
	case 1:
		idb_remove(channel->banned, tsd->status.char_id);
		sprintf(output, msg_txt(1441),tsd->status.name,chname); // Player '%s' has now been unbanned from the '%s' channel
		break;
	case 2:
		db_clear(channel->banned);
		sprintf(output, msg_txt(1442),chname); // Removed all bans from '%s' channel
		break;
	case 3: {
		...
		for( pl_sd = dbi_first(iter); dbi_exists(iter); pl_sd = dbi_next(iter) ) { //for all users
			sprintf(output, msg_txt(1444),pl_sd->status.char_id,pl_sd->status.name); //  %d: %s
			clif_displaymessage(sd->fd, output);
		}
		dbi_destroy(iter);
		}
...
int channel_pcsetopt(struct map_session_data *sd, char *chname, const char *option, const char *val){
	...
	if( option == '\0' ) {
		clif_displaymessage(sd->fd, msg_txt(1446));// You need to input a option
		return -1;
	}
	...
	if( k == 3 ) {
		sprintf(output, msg_txt(1447), option);// '%s' is not a known channel option
		clif_displaymessage(sd->fd, output);
		clif_displaymessage(sd->fd, msg_txt(1448)); // -- Available options
		for( k = 1; k < s; k++ ) {
	...
	if( val[0] == '\0' ) {
		if ( k == CHAN_OPT_MSG_DELAY ) {
			sprintf(output, msg_txt(1466), opt_str[k]);// For '%s' you need the amount of seconds (from 0 to 10)
			clif_displaymessage(sd->fd, output);
			return -1;
		} else if( channel->opt & k ) {
			sprintf(output, msg_txt(1449), opt_str[k],opt_str[k]); // option '%s' is already enabled, if you'd like to disable it type '@channel setopt %s 0'
			clif_displaymessage(sd->fd, output);
			return -1;
		} else {
			channel->opt |= k;
			sprintf(output, msg_txt(1450), opt_str[k],channel->name);//option '%s' is now enabled for channel '%s'
			clif_displaymessage(sd->fd, output);
			return 0;
		}
	} else {
		...
		if( k == CHAN_OPT_MSG_DELAY ) {
			if( v < 0 || v > 10 ) {
				sprintf(output, msg_txt(1451), v, opt_str[k]);// value '%d' for option '%s' is out of range (limit is 0-10)
				clif_displaymessage(sd->fd, output);
				return false;
			}
			if( v == 0 ) {
				channel->opt &=~ k;
				channel->msg_delay = 0;
				sprintf(output, msg_txt(1453), opt_str[k],channel->name,v);// option '%s' is now disabled for channel '%s'
				clif_displaymessage(sd->fd, output);
				return true;
			} else {
				channel->opt |= k;
				channel->msg_delay = v;
				sprintf(output, msg_txt(1452), opt_str[k],channel->name,v);// option '%s' is now enabled for channel '%s' with %d seconds
				clif_displaymessage(sd->fd, output);
				return true;
			}
		} else {
			if( v ) {
				if( channel->opt & k ) {
					sprintf(output, msg_txt(1449), opt_str[k],opt_str[k]); // option '%s' is already enabled, if you'd like to disable it type '@channel opt %s 0'
					clif_displaymessage(sd->fd, output);
					return false;
				} else {
					channel->opt |= k;
					sprintf(output, msg_txt(1454), opt_str[k],channel->name);//option '%s' is now enabled for channel '%s'
					clif_displaymessage(sd->fd, output);
				}
			} else {
				if( !(channel->opt & k) ) {
					sprintf(output, msg_txt(1454), opt_str[k],channel->name); // option '%s' is not enabled on channel '%s'
					clif_displaymessage(sd->fd, output);
					return false;
				} else {
					channel->opt &=~ k;
					sprintf(output, msg_txt(1453), opt_str[k],channel->name);// option '%s' is now disabled for channel '%s'
					clif_displaymessage(sd->fd, output);
					return true;
				}
			}
