 ...
 **/
	...
	if (sc && sc->count)
	{
		...
		if( sc->data[SC_POISONINGWEAPON] )
		{
			...
			if ( !(flag&BF_SKILL) && (flag&BF_WEAPON) && damage > 0 && rnd()%100 < sc->data[SC_POISONINGWEAPON]->val3 )
			{
				if ( sc->data[SC_POISONINGWEAPON]->val1 == 9 ) {//Oblivion Curse gives a 2nd success chance after the 1st one passes which is reduceable. [Rytech]
					rate = 100 - tstatus->int_ * 4 / 5 ;
					if ( rate < 5 ) rate = 5;
				}
				sc_start(bl,sc->data[SC_POISONINGWEAPON]->val2,rate,sc->data[SC_POISONINGWEAPON]->val1,skill_get_time2(GC_POISONINGWEAPON, 1) - (tstatus->vit + tstatus->luk) / 2 * 1000);
			}
		}
	...
	case CR_ACIDDEMONSTRATION:
#ifdef RENEWAL
		{
			...
			if ((atk != 0 && size_mod != 0) || matk != 0)
				md.damage = (int)(int64)(((7 * atk * tstatus->vit) * size_mod / 100 + (7 * matk * tstatus->vit)) / 100);
			else
				md.damage = 0;
	...
	case RA_FIRINGTRAP:
 	case RA_ICEBOUNDTRAP:
		...
		if (sd) {
			...
			if(researchSkillLv)
				md.damage = (int64)md.damage * 20 * researchSkillLv / (skill_id == RA_CLUSTERBOMB?50:100);
			else
				md.damage = 0;
		} else
		...
		if( sc && sc->count ) {
			...
			if(sc->data[SC_DEATHBOUND] && skill_id != WS_CARTTERMINATION && !(src->type == BL_MOB && is_boss(src)) ) {
				...
				if( distance_bl(src,bl) <= 0 || !map_check_dir(dir,t_dir) ) {
					...
					*dmg = (int64)rd1 * 30 / 100; // Player receives 30% of the amplified damage.
