...
static short status_calc_aspd(struct block_list *bl, struct status_change *sc, short flag)
{
	...
	if(sc->data[SC_BERSERK] &&	skills1 < 15)
		...
		skills1 = 15;
	else if((sc->data[SC_GUST_OPTION] || sc->data[SC_BLAST_OPTION] || sc->data[SC_WILD_STORM_OPTION]) && skills1 < 5)
		skills1 = 5;
...
static unsigned int status_calc_maxhp(struct block_list *bl, struct status_change *sc, unsigned int maxhp)
{
	...
	if(sc->data[SC__INVISIBILITY])
		maxhp -= maxhp * sc->data[SC__INVISIBILITY]->val4 / 100;
		...
		case SC_GT_CHANGE:
			{// take note there is no def increase as skill desc says. [malufett]
				...
				if( (src = map_id2bl(val2)) )
					val4 = ( 200/status_get_int(src) ) * val1;// MDEF decrease: MDEF [(200 / Casters INT) x Skill Level]
			}
			break;
		case SC_GT_REVITALIZE:
