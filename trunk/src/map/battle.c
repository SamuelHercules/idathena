	...
	switch( attack_type ) {
		...
		case BF_WEAPON:
			...
			if( sd && !(nk&NK_NO_CARDFIX_ATK) && (left&2) ) { //Attacker cards should be checked
				...
				//Add check here for Dragon Breath, because we want to apply the same behavior in pre-renewal [exneval]
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc && sc->count ) {
		...
		if( (sce = sc->data[SC_AUTOGUARD]) && ((flag&BF_WEAPON) || skill_id == GN_HELLS_PLANT_ATK) &&
		...
		if( (sce = sc->data[SC_KYRIE]) && damage > 0 ) {
			sce->val2 -= (int)cap_value(damage,INT_MIN,INT_MAX);
			if( (flag&BF_WEAPON) || skill_id == TF_THROWSTONE || skill_id == GN_HELLS_PLANT_ATK ) {
				if( sce->val2 >= 0 )
					damage = 0;
				else
				  	damage = -sce->val2;
